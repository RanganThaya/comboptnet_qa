...
def compute_delta_y(y, y_grad, lb, ub, clip_gradients_to_box, use_canonical_basis):
    ...
    if clip_gradients_to_box:
        ...
        inside_box_indicator = jnp.greater_equal(y_test, lb) * jnp.less_equal(
            y_test, ub
        )
    ...
    if use_canonical_basis:
        ...
        permuted_triangular_matrix = jnp.take_along_axis(
            triangular_matrix[None, :, :], ranks_of_abs_y_grad[:, None, :], axis=-1
        )

        delta_y = (
            permuted_triangular_matrix * jnp.sign(y_grad + epsilon_constant)[:, None, :]
        )

        abs_grad_sorted = jnp.take_along_axis(jnp.abs(y_grad), sort_indices, axis=-1)
        sorted_grad_zero_append = jnp.concatenate(
            (jnp.zeros((bs, 1)), abs_grad_sorted), axis=-1
        )
...
def signed_euclidean_distance_constraint_point(constraints, point):
    constraint_lhs = (
        jnp.sum(constraints[..., :-1] * point[..., None, :], axis=-1)
        + constraints[..., -1]
    )
    distance_point_const = constraint_lhs / (
        jnp.linalg.norm(constraints[..., :-1], axis=-1) + epsilon_constant
    )
    return distance_point_const


def check_point_feasibility(point, constraints, lb, ub):
    distance_point_const = signed_euclidean_distance_constraint_point(
        constraints=constraints, point=point
    )
    feasibility_indicator_constraints = jnp.all(
        jnp.less_equal(distance_point_const, 0.0), axis=-1
    )
