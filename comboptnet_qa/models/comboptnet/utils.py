...
def compute_metrics(y, y_true, y_uncon=None, prefix="", acc_threshold=1e-5):
    ...
    if y_uncon is not None:
        ...
        match_uncon, match_uncon_perfect = check_equal_ys(
            y, y_uncon, threshold=acc_threshold
        )
        metric_dict.update(
            dict(
                match_uncon_accuracy=match_uncon,
                match_uncon_perfect_accuracy=match_uncon_perfect,
            )
        )
...
def check_if_zero_or_one(y):
    return torch.all(
        torch.logical_or(
            torch.isclose(y, torch.ones_like(y)), torch.isclose(y, torch.zeros_like(y))
        )
    )
...
class ParallelProcessing:
    ...
    def maybe_parallelize(self, function, static_kwargs, dynamic_kwargs):
        if self.ray_available():
            ...
            return self.ray.get(
                [remote_fn(**static_kwargs, **kwargs) for kwargs in dynamic_kwargs]
            )
        else:
...
def solve_unconstrained(cost_vector, lb, ub):
    ...
    if isinstance(cost_vector, torch.Tensor):
        indicator = (cost_vector >= 0).to(torch.float) - (cost_vector <= 0).to(
            torch.float
        )
    else:
...
def print_train_acc(metrics, epoch):
    print(
        f"Epoch: {epoch + 1:>2}, Train loss: {metrics['train_loss']:.4f}, "
