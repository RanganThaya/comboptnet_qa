...
from comboptnet_qa.models.explanationlp.single_model import SingleModel
from comboptnet_qa.tasks.graph_construction.explanationlp_graph_construction import (
    CandidateGraphConstruction,
)
from comboptnet_qa.tasks.graph_construction.single_graph_construction import (
    SingleGraphConstruction,
)
...
graph_extraction = SingleGraphConstruction()
...
def dataset_construction(split="test", dataset="arc", **opts):
    ...
    with Flow("Question answering with WorldTreeCorpus") as flow:
        ...
        with tags("question_transformer" + split + str(TR_MODEL) + str(HYP_MAX_LEN)):
        ...
        with tags("table_store abstract" + str(TR_MODEL) + " " + str(FACT_MAX_LEN)):
...
@task
def init_model(disable_transformer=False, only_answer=False, opts={"lambda_val": 50}):
    model = SingleModel(
        transformer_model=TR_MODEL,
        num_nodes=LIMIT,
        hyp_max_len=HYP_MAX_LEN,
