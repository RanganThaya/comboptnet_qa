...
TASK_NAME = f"tuple_ilp_corpus_retrieval_abductive"
cache_args = dict(
...
@task(**cache_args)
def corpus_retrieval(hypothesis, k=100):
    # Build the flow
    with Flow("Explanation selection with Tablestore") as flow:
        corpus = table_store_extraction_task(table_store_path, original_map=True)
