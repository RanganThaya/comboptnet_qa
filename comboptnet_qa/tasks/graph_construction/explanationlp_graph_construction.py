...
class CandidateGraphConstruction(Task):
    ...
    def construct_graphs(
        ...
        NO_OF_NODES = num_nodes + num_choices
        explanation_graphs = {}
        for q_id, q_exp in tqdm(input.items()):
            ...
            for c_index, choice in enumerate(q_exp["choices"].values()):
                ...
                for a_id, score in combined_abstract_facts.items():
                    ...
                    if not overlap_score > 0:
                        continue
                    q_ovrlp_trms[c_id][a_id] = overlap_terms
                    ...
                    selected_abstract_nodes[a_id] = max(
                        a_sem_score, selected_abstract_nodes[a_id]
                    )
                    # selected_abstract_nodes[a_id] = max(
                    # score, selected_abstract_nodes[a_id]
                    # )
                    abstract_nodes[c_id][a_id] = score
            ...
            # max_grounding_node_count = num_nodes - len(selected_abstract_nodes)
            ...
            for c_index, choice in enumerate(q_exp["choices"].values()):
                if c_index >= num_choices:
                    ...
                    continue
                c_id = f"{q_id}|{choice}"
                grounding_nodes[c_id] = {}
                grounding_lexical_nodes[c_id] = {}
                for g_id, fact in table_store.items():
                    if fact["type"] == "ABSTRACT":
                        continue

                    g_score = 1
                    if len(grounding_nodes[c_id]) > max_grounding_node_count - 1:
                        break
                    (
                        overlap_score,
                        overlap_terms,
                    ...
                    if g_score > 0 and overlap_score > 0:
                        ...
                        if not has_abstract_overlap:
                            continue
                        q_ovrlp_trms[c_id][g_id] = overlap_terms
                        grounding_nodes[c_id][g_id] = 1
                        grounding_lexical_nodes[c_id][g_id] = g_score
                        inference_chains.append(
                            (
                                c_id,
                                g_id,
            ...
            recall = 0
            if "explanation" in q_exp and len(q_exp["explanation"]) > 0:
                for t_id in node_mapping:
                    if (
                        t_id
                        in q_exp["explanation"]
                        ...
                        # ] = 1
                if recall < 1:
                    logger.warning(f"Only able to retrieve {recall} for {q_id}")
                if len(q_exp["explanation"]) > 0:
                    recall = recall / len(q_exp["explanation"])

            assert (
                len(node_mapping) + num_choices <= NO_OF_NODES
            ), f"{len(node_mapping)}"
            # for id, index in node_mapping.items():
            #     if id in selected_abstract_nodes:
            #         for c_id, a_node_mapping in abstract_nodes.items():
            #             if id in a_node_mapping:
            #                 abstract_facts[
            #                     choice_mapping[c_id], index
            #                 ] = selected_abstract_nodes[id]
            #     elif id in selected_grounding_node:
            #         for c_id, g_node_mapping in grounding_nodes.items():
            #             if id in g_node_mapping:
            #                 grounding_facts[
            #                     choice_mapping[c_id], index
            #                 ] = selected_grounding_node[id]
            ...
            for chain in inference_chains:
                ...
                if c_type == "abstract" and t_id in selected_abstract_nodes:
                    question_abstract_edges[
                        choice_mapping[c_id], node_mapping[t_id]
                    ] = score
            ...
            for a1_id in selected_abstract_nodes:
                for a2_id in selected_abstract_nodes:
                    if a1_id != a2_id:
                        for ct_id in choice_mapping:
                            if (
                                a1_id in q_ovrlp_trms[ct_id]
                                and a2_id in q_ovrlp_trms[ct_id]
                            ):
                                score, _ = CandidateGraphConstruction.calc_interoverlap(
                                    q_ovrlp_trms[ct_id][a1_id],
                                    q_ovrlp_trms[ct_id][a2_id],
                                )
            ...
            for g_id, a_id in grounding_abstract_overlap_maps:
                ...
                if g_id in selected_grounding_node and a_id in selected_abstract_nodes:
            ...
            explanation_graphs[q_id]["is_abstract"] = is_abstract
            ...
            explanation_graphs[q_id]["choice_mapping"] = {
                val: key for val, key in choice_mapping.items()
            }
            explanation_graphs[q_id]["similarity_scores"] = similarity_scores
            explanation_graphs[q_id]["recall"] = recall
...
class TorchDataset(Dataset):
    ...
    def build_constraints(
        ...
        NO_OF_NODES = self.num_nodes + self.num_choices
        num_variables = NO_OF_NODES * NO_OF_NODES

        # root constraint
        root_constraint_A = torch.zeros((1, NO_OF_NODES * NO_OF_NODES))
        root_constraint_A2 = torch.zeros((1, NO_OF_NODES * NO_OF_NODES))
        for i in range(self.num_choices):
            root_constraint_A[0, i * NO_OF_NODES + i] = 1
            root_constraint_A2[0, i * NO_OF_NODES + i] = -1
    ...
    def __getitem__(self, index):
        ...
        fact_input_ids = np.zeros((self.num_nodes, self.fact_max_len), dtype=np.long)
        fact_attention_mask = np.zeros(
            (self.num_nodes, self.fact_max_len), dtype=np.long
        )
        ...
        results = self.explanation_graphs[self.key_map[index]]
        similarity_scores = results["similarity_scores"]

        mapping = np.concatenate((answer, similarity_scores)).reshape(
            (self.num_nodes + self.num_choices, 1)
        )
        ...
        for f_index, f_id in results["node_mapping"].items():
            assert f_index != 0
            fact_input_ids[f_index - self.num_choices] = self.encoded_table_store[f_id][
                "input_ids"
            ]
            fact_attention_mask[f_index - self.num_choices] = self.encoded_table_store[
                f_id
            ]["attention_masks"]
        for i in range(self.num_nodes - len(results["node_mapping"])):
            fact_input_ids[len(results["node_mapping"]) + i][0] = 101
            fact_input_ids[len(results["node_mapping"]) + i][1] = 102
            fact_attention_mask[len(results["node_mapping"]) + i][0] = 1
            fact_attention_mask[len(results["node_mapping"]) + i][1] = 1
        ...
        abstract_abstract_lexical = results["abstract_abstract_overlap_edges"].toarray()
        abstract_abstract_similarity = results["abstract_abstract_edges"].toarray()
